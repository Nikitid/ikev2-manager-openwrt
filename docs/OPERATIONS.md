...
derived routing state. Every 20 seconds it sends a small HTTPS probe through
`ipsec-out`, trying Cloudflare and then Amazon as a fallback; two consecutive
failed probe cycles trigger the same serialized reconnect action used by LuCI.
This catches a CHILD_SA that remains installed but no longer forwards packets
without reacting to one transient endpoint timeout. If a rekey changes the assigned virtual IPv4, stale
