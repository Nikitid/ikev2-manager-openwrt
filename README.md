...
- конкурирующее действие быстро отклоняется, а списки и статусы обновляются без
  ручной перезагрузки страницы;
- полный сброс зависимостей сначала проверяет восстановление DNS, удаляет только
  установленные приложением пакеты и сохраняет общие пакеты других приложений;
